...
function phase_aes_ptable() {
    cp $AES_CONF/Parameters.table.merge $AES_SERVER_HOME/config/Parameters.table.merge
    cd $AES_SERVER_HOME 
    run_cmd_err bin/tableedit -script $AES_CONF/script.table
    rm $AES_SERVER_HOME/config/Parameters.table.merge
}
